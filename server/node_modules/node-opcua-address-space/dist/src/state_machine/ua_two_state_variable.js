...
exports._addTwoStateVariable = exports.UATwoStateVariable = exports.promoteToTwoStateVariable = exports._install_TwoStateVariable_machinery = void 0;
/**
...
function _getHumanReadableString(node) {
    ...
    if (dataValue.statusCode !== node_opcua_status_code_1.StatusCodes.Good) {
        ...
        _c.value = new node_opcua_variant_1.Variant({
            ...
            value: node_opcua_data_model_1.coerceLocalizedText("")
        });
        return _c;
    }
...
function promoteToTwoStateVariable(node) {
    ...
    if (!(node instanceof ua_variable_1.UAVariable)) {
...
register_node_promoter_1.registerNodePromoter(node_opcua_constants_1.VariableTypeIds.TwoStateVariableType, promoteToTwoStateVariable);
/***
...
class UATwoStateVariable extends ua_variable_1.UAVariable {
    ...
    initialize(options) {
        ...
        if (options.trueState) {
            ...
            if (node.falseState) {
                node.falseState.setValueFromSource({
                    dataType: node_opcua_variant_2.DataType.LocalizedText,
                    value: node_opcua_data_model_1.coerceLocalizedText(options.falseState)
                });
            }
            else {
                node._trueState = node_opcua_data_model_1.coerceLocalizedText(options.trueState).text;
            }
            if (node.trueState) {
                node.trueState.setValueFromSource({
                    dataType: node_opcua_variant_2.DataType.LocalizedText,
                    value: node_opcua_data_model_1.coerceLocalizedText(options.trueState)
                });
            }
            else {
                node._falseState = node_opcua_data_model_1.coerceLocalizedText(options.falseState).text;
            }
        }
        ...
        if (options.value === undefined) {
            ...
            node_opcua_assert_1.assert(options.value.dataType === node_opcua_variant_2.DataType.Boolean);
            node.id.setValueFromSource(options.value, node_opcua_status_code_1.StatusCodes.Good);
        }
        else {
    ...
    getTrueState() {
        return this.trueState ? this.trueState.readValue().value.value : node_opcua_data_model_1.coerceLocalizedText(this._trueState || "TRUE");
    }
    getFalseState() {
        return this.falseState ? this.falseState.readValue().value.value : node_opcua_data_model_1.coerceLocalizedText(this._falseState || "FALSE");
    }
...
function _addTwoStateVariable(namespace, options) {
    ...
    const node = twoStateVariableType.instantiate({
        ...
        dataType: node_opcua_nodeid_1.resolveNodeId(node_opcua_variant_2.DataType.LocalizedText),
